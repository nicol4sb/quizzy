...
async function loadQuiz(database: Database, quizId: string, creatorId: string) {
  const result = await database.query<QuizRow>(
    `SELECT q.id AS quiz_id, q.title, q.theme, q.created_at, q.updated_at,
...
export async function registerQuizRoutes(
  ...
  app.get("/api/quizzes", async (request, reply) => {
    ...
    const result = await database.query(
      `SELECT q.id, q.title, q.theme, q.created_at AS "createdAt", q.updated_at AS "updatedAt",
  ...
  app.post("/api/quizzes", async (request, reply) => {
    ...
    try {
      ...
      await client.query(
        "INSERT INTO quizzes (id, creator_id, title, theme) VALUES ($1, $2, $3, $4)",
        [quizId, creatorId, parsed.data.title, parsed.data.theme],
      );
  ...
  app.put("/api/quizzes/:id", async (request, reply) => {
    ...
    try {
      ...
      await client.query(
        `UPDATE quizzes SET title = $1, theme = $2, updated_at = now()
          WHERE id = $3 AND creator_id = $4`,
        [parsed.data.title, parsed.data.theme, parsedId.data, creatorId],
      );
