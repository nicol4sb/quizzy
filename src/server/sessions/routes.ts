...
type PlayerRow = { id: string; nickname: string; joined_at: Date };

function publicPlayer(row: PlayerRow) {
  return { id: row.id, nickname: row.nickname, joinedAt: row.joined_at };
}

async function playersFor(database: Database, sessionId: string) {
...
export async function registerSessionRoutes(
    ...
    try {
      ...
      for (let attempt = 0; attempt < 10 && !created; attempt += 1) {
        ...
        const result = await client.query<SessionRow>(
          `INSERT INTO live_sessions (id, quiz_id, host_creator_id, join_code)
           VALUES ($1, $2, $3, $4)
           ON CONFLICT (join_code) DO NOTHING
           RETURNING id, quiz_id, (SELECT title FROM quizzes WHERE id = $2) AS quiz_title,
                     join_code, state, revision, created_at`,
          [sessionId, parsedId.data, hostCreatorId, code],
        );
  ...
  app.get("/api/sessions/:id/host", async (request, reply) => {
    ...
    const result = await database.query<SessionRow>(
      `${sessionSelect} WHERE s.id = $1 AND s.host_creator_id = $2`,
      [parsedId.data, hostCreatorId],
    );
    const session = result.rows[0];
    if (!session) return reply.code(404).send({ error: "Session not found." });
    const question = ["QUESTION_OPEN", "RESULTS", "FINISHED"].includes(
      session.state,
    )
  ...
  app.get("/api/sessions/:id/player", async (request, reply) => {
    ...
    if (!player) return reply.code(401).send({ error: "Not authorized." });
    const result = await database.query<SessionRow>(
    ...
    return {
      ...
      currentQuestion: ["QUESTION_OPEN", "RESULTS", "FINISHED"].includes(
        session.state,
      )
        ? await currentQuestion(database, session.id)
        : undefined,
  ...
  app.get("/api/lobbies/:code", async (request, reply) => {
    const parsedCode = joinCodeSchema.safeParse(
      (request.params as { code?: string }).code,
    );
    if (!parsedCode.success)
      return reply.code(404).send({ error: "Lobby not found." });
    const result = await database.query<SessionRow>(
      `${sessionSelect} WHERE s.join_code = $1 AND s.state = 'LOBBY'`,
      [parsedCode.data],
    );
    const session = result.rows[0];
    return session
  ...
  app.post("/api/lobbies/:code/players", async (request, reply) => {
    ...
    try {
      ...
      const found = await client.query<SessionRow>(
        `${sessionSelect} WHERE s.join_code = $1 FOR UPDATE OF s`,
        [parsedCode.data],
      );
      ...
      const inserted = await client.query<PlayerRow>(
        `INSERT INTO players (id, live_session_id, nickname, token_hash)
         VALUES ($1, $2, $3, $4)
  ...
  app.post("/api/sessions/:id/start", async (request, reply) => {
    ...
    try {
      ...
      const playerCount = await client.query(
        "SELECT 1 FROM players WHERE live_session_id = $1 LIMIT 1",
        [session.id],
      );
      ...
      if (!question) throw new Error("Playable quiz has no first question");
      await client.query(
  ...
  app.post("/api/sessions/:id/answers", async (request, reply) => {
    ...
    try {
      ...
      const roundResult = await client.query<{
        ...
        `SELECT s.revision, s.state, r.opened_at, r.closes_at, now() AS server_now,
          ...
          FOR UPDATE OF s`,
        [parsedId.data, parsedBody.data.roundId, parsedBody.data.answerId],
      );
      ...
      const existing = await client.query<{
        ...
        `SELECT id, answer_option_id
           FROM answer_submissions
          WHERE question_round_id = $1 AND player_id = $2`,
        [parsedBody.data.roundId, player.id],
      );
      if (existing.rows[0]) {
        await client.query("ROLLBACK");
        transactionOpen = false;
      ...
      const progressResult = await client.query<{
        ...
        `SELECT
           ...
           (SELECT count(*)::integer FROM players WHERE live_session_id = $1) AS total_players`,
        [parsedId.data, parsedBody.data.roundId],
      );
      const counts = progressResult.rows[0]!;
      let resultsRevision: number | undefined;
      if (
        counts.total_players > 0 &&
        counts.answered_count === counts.total_players
      ) {
        await client.query(
          "UPDATE question_rounds SET closed_at = now() WHERE id = $1",
          [parsedBody.data.roundId],
        );
        const updated = await client.query<{ revision: number }>(
          `UPDATE live_sessions
      ...
      if (resultsRevision !== undefined) {
        ...
        if (!results) throw new Error("Automatic results could not be read");
        await app.roomEventBus.publish(parsedId.data, {
  ...
  app.post("/api/sessions/:id/reveal", async (request, reply) => {
    ...
    try {
      ...
      if (!results) throw new Error("Revealed results could not be read");
      await app.roomEventBus.publish(session.id, {
        type: "results_revealed",
        revision: updated.rows[0]!.revision,
        payload: { results },
  ...
  app.post("/api/sessions/:id/next", async (request, reply) => {
    ...
    try {
      ...
      const next = await client.query<{
        ...
        `SELECT q.id, q.position, q.time_limit_seconds
           FROM questions q
          WHERE q.quiz_id = $1
      ...
      const publicQuestion = await currentQuestion(database, session.id);
      if (!publicQuestion) throw new Error("Next question could not be read");
      await app.roomEventBus.publish(session.id, {
        type: "question_opened",
        revision: updated.rows[0]!.revision,
        payload: { question: publicQuestion },
