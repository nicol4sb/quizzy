...
const eventSchema = z
    ...
    .strict();
async function currentAdmin(request, database) {
...
export async function recordAnalyticsEvent(database, event) {
    try {
        await database.query(`INSERT INTO analytics_events
...
export async function registerAnalyticsRoutes(app, database) {
    ...
    app.get("/api/admin/analytics", async (request, reply) => {
        ...
        const rawDays = Number(new URL(request.url, "http://localhost").searchParams.get("days") ?? 30);
        const days = Number.isFinite(rawDays)
            ? Math.min(90, Math.max(1, Math.floor(rawDays)))
            : 30;
        const interval = `${days} days`;
        const [overview, daily, paths, eventTypes, quizzes, recent] = await Promise.all([
          ...
          WHERE occurred_at >= now() - $1::interval`, [interval]),
            database.query(`SELECT occurred_at::date::text AS day,
                  count(*) FILTER (WHERE event_type = 'page_view')::int AS visits,
                  count(*)::int AS events
             FROM analytics_events
            WHERE occurred_at >= now() - $1::interval
            GROUP BY occurred_at::date
            ORDER BY occurred_at::date`, [interval]),
            database.query(`SELECT COALESCE(path, '/') AS path, count(*)::int AS views
             FROM analytics_events
            ...
            LIMIT 12`, [interval]),
            database.query(`SELECT event_type, count(*)::int AS count
             FROM analytics_events
            WHERE occurred_at >= now() - $1::interval
            GROUP BY event_type
            ORDER BY count DESC, event_type
            LIMIT 16`, [interval]),
            database.query(`SELECT q.id, q.title, q.play_count,
            ...
            LIMIT 20`, [interval]),
            database.query(`SELECT e.event_type, e.path, e.occurred_at, q.title AS quiz_title
