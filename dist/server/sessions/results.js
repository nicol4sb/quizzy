...
export async function leaderboard(database, sessionId) {
    const result = await database.query(`SELECT p.id, p.nickname, p.joined_at,
      ...
      ORDER BY score DESC, correct_answers DESC, p.joined_at, p.id`, [sessionId]);
...
export async function questionResults(database, sessionId) {
    const question = await currentQuestion(database, sessionId);
    if (!question)
        return undefined;
    const answers = await database.query(`SELECT option.id, option.is_correct,
      ...
      ORDER BY option.position`, [question.roundId]);
    const correct = answers.rows.find((answer) => answer.is_correct);
    if (!correct)
        throw new Error("Question has no correct answer");
    const totalResult = await database.query(`SELECT count(*)::integer AS total_players
       FROM players WHERE live_session_id = $1`, [sessionId]);
    ...
    return {
        ...
        voteTotals,
        answeredCount: voteTotals.reduce((total, answer) => total + answer.count, 0),
